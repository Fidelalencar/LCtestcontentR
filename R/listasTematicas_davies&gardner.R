...
# RETIRADO DO Dicionário de frequencia de palavras de Davies & Gardner

# 1. listas de palavras tematicas
# 2. listas de frequencia por aspectos do idioma
# 3. Listas das 5000 extraidas do livro ()
# 4. Lista das 5000 e suas collocations (palavras que mais frequentemente vêm acompanhadas)
...
# criando lista de todas as palavras (usando os nomes dos vetores como tags da lista) : Aqui é Facil com tibble
...
family <- c('child', 'mother', 'father', 'kid', 'parent', 'wife', 'son', 'baby', 'brother', 'husband', 'daughter', 'sister', 
            ...
            'godfather', 'fiancé', 'soninlaw', 'sister-in-law', 'fiancée', 'father-in-law', 'grandchild', 'daughter-in-law', 'triplet', 'widower', 'foster child', 'godmother')
...
# PASSO IMPORTANTE: 
# hash_lemmas só tem a lista das palavras modificadas. Ou seja, as palavras base só estão na segunda coluna (lemma), não na primeira (token)
...
# extraindo as colunas que não interessam temporariamente
...
# 1. foi fundamental adicionar a 'hash_lemmas'  lista das palavras modificadas. Ou seja, as palavras "base" só estavam na segunda coluna (lemma), não na primeira (token)
# 2. muitas palavras de them são "expressões", termos que acho que realmente não faziam sentido estar em hash_lemmas. Portanto, a lista 'them' cresce, mas com muitas palavras não presentes em hash_lemmas 
...
## RAPIDO ADENDO: duas funções interessantes para estudar match: match() [retorna indices] e %in% [retorna valores logicos]
# 
#
...
voc_newspapers <- c('game', 'team', 'season', 'player', 'official', 'county', 'director', 'executive', 'district', 'coach', 'league', 'manager', 
                    ...
                    'average', 'oversee', 'staff', 'total', 'commute', 'email', 'bat', 'rebound', 'single', 'preheat', 'lease', 'sauté', 'renovate', 'refrigerate', 
...
'mentor', 'morph', 'vaccinate', 'restart', 'reconnect', 'sauté', 'hijack', 'cowrite', 'ditch', 'reference', 'swipe', 'outsource', 'transition', 
...
# alguns collectives eu não entendo....
...
#### criando tábua (lista) de todas as palavras
...
####
# criando tábua de todas as palavras (aqui, as "classes" saem numeradas por causa da func unlist())
...
div<-diversity(v_token1$v_token1)  # Aqui se obtem os índices de dispersão: é possivel colocar vai de um vetor (de textos) para comparar os textos

dispersion_plot(v_token1$v_token1, matches) # aqui se visualiza a dispersão: também é possivel colocar mais de um vetor para comparar textos



## 3. Outra tentativa de Lexical Dispersion Plot
# ver no último 1/4 do link: http://trinker.github.io/qdap/vignettes/qdap_vignette.html
...
with(rajSPLIT, dispersion_plot(dialogue, c("love", "night"),
                               ...
                               color = "yellow", 
                               total.color = "white", # cor da dispersão na linha de todas as palavras
                               horiz.color="grey20"))

# buscando a dispersão lexical das palavras mais frequentes retirando as stopwords
wrds <- freq_terms(pres_debates2012$dialogue, stopwords = Top200Words) #frequencia retirando as stopwords
wrds2 <- spaste(wrds) # Adicionando espaços (ver: leading/trailing), caso necessario
